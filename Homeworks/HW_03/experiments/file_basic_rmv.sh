...
printf "    ${GREEN}[*]${NC} Creating One 4KiB File with the Same Content (Perl Script)...\n"
FILE_THREE='FILE_THREE'
perl -e 'print "A" x 4096' > $FILE_THREE
...
#Delete FILE_ONE
printf "    ${GREEN}[*]${NC} Deleting $FILE_ONE (BB_UNLINK)...\n" ### Delete
rm "$mountdir/$FILE_ONE"
...
#Delete One FILE_TWO
printf "    ${GREEN}[*]${NC} Deleting $FILE_THREE (BB_UNLINK)...\n" ### Delete
rm "$mountdir/$FILE_THREE"
...
#Delete FILE_THREE
printf "    ${GREEN}[*]${NC} Deleting $FILE_TWO (BB_UNLINK)...\n" ### Delete
rm "$mountdir/$FILE_TWO"
